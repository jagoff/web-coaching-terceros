...
test.describe('LaVague Advanced AI - Natural Language Understanding', () => {
  ...
  test('AI understands and describes the page purpose in natural language', async ({ page }) => {
    ...
    const pageAnalysis = await page.evaluate(() => {
      ...
      const services = Array.from(
        document.querySelectorAll('[class*="service"], [class*="offer"]')
      ).map(el => ({
  ...
  test('AI identifies and classifies all conversion opportunities', async ({ page }) => {
    ...
    const conversionAnalysis = await page.evaluate(() => {
      ...
      // CTA buttons
      document
        .querySelectorAll('button, .btn, a[href*="contact"], a[href*="agenda"]')
        .forEach((el: Element) => {
      ...
      document
        .querySelectorAll(
          'a[href*="instagram"], a[href*="linkedin"], a[href*="facebook"], a[href*="twitter"]'
        )
        .forEach((el: Element) => {
          conversionElements.push({
            type: 'Social Media',
            text:
              (el.textContent || '').trim() ||
              (el as HTMLAnchorElement).href.split('/').pop() ||
              'Social Link',
            position: el.getBoundingClientRect().top,
            visible: (el as HTMLElement).offsetParent !== null,
          })
    ...
    conversionAnalysis.forEach((element, index) => {
      console.log(
        `   ${index + 1}. ${element.type}: "${element.text}" (Visible: ${element.visible})`
      )
    })
    ...
    const funnelStages = {
      ...
      consideration: conversionAnalysis.filter(
        (e: any) => e.type === 'CTA Button' && e.text.includes('más')
      ).length,
      conversion: conversionAnalysis.filter(
        (e: any) => e.type === 'Contact Form' || e.text.includes('agenda')
      ).length,
  ...
  test('AI performs semantic content analysis and quality assessment', async ({ page }) => {
    ...
    console.log(
      '   Vocabulary Richness:',
      ((contentAnalysis.uniqueWords / contentAnalysis.totalWordCount) * 100).toFixed(1) + '%'
    )
  ...
  test('AI simulates user journey with decision making', async ({ page }) => {
    ...
    if (services.length > 0 && testimoniosFound > 0) {
      journeyLog.push(
        '🧠 AI Conclusion: Page has good services and social proof - ready for conversion'
      )

      // AI Decision: Look for easiest conversion path
      const freeSessionCTA = page
        .locator('a, button')
        .filter({ hasText: /sesión.*gratis|gratuita/ })
        .first()
  ...
  test('AI generates natural language recommendations', async ({ page }) => {
    ...
    const siteAnalysis = await page.evaluate(() => {
      return {
        hasVideo:
          ...
          0,
        hasTestimonials:
          document.querySelectorAll('[class*="testimonio"], [id*="testimonio"]').length > 0,
        hasPricing:
          document.querySelectorAll('[class*="precio"], [class*="price"], [id*="precio"]').length >
          0,
        hasFAQ:
          document.querySelectorAll('[class*="pregunta"], [class*="faq"], [id*="faq"]').length > 0,
        ...
        hasCertifications:
          document.querySelectorAll('[class*="certificacion"], [class*="certificado"]').length > 0,
      }
    })
    ...
    if (!siteAnalysis.hasPricing) {
      recommendations.push(
        'Add transparent pricing information to reduce friction in the decision process'
      )
    }
    ...
    if (siteAnalysis.ctaCount < 3) {
      recommendations.push(
        'Add more strategic CTA buttons throughout the page to capture user interest at different stages'
      )
    }

    if (!siteAnalysis.hasBlog) {
      recommendations.push(
        'Consider adding a blog or resources section to demonstrate expertise and improve SEO'
      )
    }
    ...
    if (siteAnalysis.formCount === 0) {
      recommendations.push(
        'Add a contact form to capture leads who prefer not to call or email directly'
      )
    }
    ...
    if (parseInt(percentage) >= 80) {
      console.log(
        '🧠 AI Assessment: Excellent! Your site is well-structured and conversion-focused'
      )
    } else if (parseInt(percentage) >= 60) {
      console.log('🧠 AI Assessment: Good foundation with room for improvement')
    } else {
      console.log(
        '🧠 AI Assessment: Basic structure present - consider implementing the recommendations above'
      )
    }
