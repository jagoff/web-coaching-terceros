      ...
      <div className="flex flex-col items-center gap-2">
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            {showPercentage && (
              <span className="text-xs font-medium text-white">{Math.round(currentProgress)}%</span>
            )}
          </div>
        </motion.div>
      </div>
  ...
  if (variant === 'dots') {
    return (
      <div className="flex items-center gap-2">
        ...
        {showPercentage && (
          <span className="text-sm text-gray-400 ml-2">{Math.round(currentProgress)}%</span>
        )}
      </div>
    )
  }
    ...
    <div className="w-full">
      ...
      {showPercentage && (
        <div className="mt-2 text-center">
          <span className="text-sm text-gray-400">{Math.round(currentProgress)}%</span>
        </div>
      )}
    </div>
  )
      ...
      {isLoading && (
        ...
        >
          <div className="max-w-md mx-auto">
            <LoadingProgress progress={progress} showPercentage={false} variant="line" size="sm" />
          </div>
        </motion.div>
      )}
    </AnimatePresence>
