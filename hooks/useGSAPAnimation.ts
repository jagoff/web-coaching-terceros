...
export const useGSAPAnimation = () => {
  ...
  const registerElements = useCallback(
    (elements: { [key: string]: Element | Element[] | null }) => {
  ...
  const animate = useCallback((selector: string | Element | Element[], config: AnimationConfig) => {
    const elements =
    ...
    // Crear animación
    const tween = gsap.fromTo(elements, from || { opacity: 0, y: 50 }, {
      ...to,
      opacity: 1,
      y: 0,
  ...
  const staggerAnimate = useCallback((elements: (string | Element)[], config: AnimationConfig) => {
  ...
  const animateOnScroll = useCallback(
    (selector: string | Element | Element[], config: AnimationConfig) => {
